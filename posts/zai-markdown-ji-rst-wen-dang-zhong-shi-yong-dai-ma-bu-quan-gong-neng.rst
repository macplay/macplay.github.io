...
.. raw:: html
   ...
   <video src="https://github.com/ashfinal/bindata/raw/master/videos/ncm_scope.mp4" loop autoplay>
       Your browser does not support the video tag.
   </video>

从上图中可以看到 ncm_ 的一项特色功能： `Scoping` 。 `Scoping` 允许你直接在某 `作用域（Scope）` 内补全 Python、JavaScript、Go……等等语言，即 ncm_ 框架下的所有语言。笔者接触到 ncm_ 时已有 Markdown 支持，两天前寻思增加 reStructuredText 应该也不会太难，就尝试着提了个 issue 请求。结果 ncm_ 作者反应异常迅速，当天就增加了对 reStructuredText 代码区块的支持。考虑到大部分受众恐怕还在使用 Markdown，上图录屏中仍采用了 Markdown 文档来做示范，以让读者有最直观的感受。
